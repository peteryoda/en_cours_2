...
# path_input_files = "/carrefour_bio/fichiers_générés_par_Hervé"
# url_one_product = "https://www.greenweez.com/lima-chips-aux-lentilles-original-90g-p83002"
# url_one_product = "https://naturalia.fr/290g-pure-orang-bio-vitagermin"
# threshold_value = 0.75
# threshold_value = None
# method_chosen = "new_one"

# Case 3
path_input_files = "/carrefour_chunking_bio/fichiers_générés_par_Hervé"
...
def main(url_product, threshold, method):
    ...
    if method == "all_except_one" or method == "all":
    ...
    # elif path_input != None and method == "new_one":
        ...
        orgc_descriptions_all_slugs_except_one = step_1.create_df("orgc_all_slugs_product", data_folder_input + "/all/", "utf8")
    ...
    if method == "new_one":
        ...
        # --- * on enlève dans les titres sans stopwords et sans année les séparateurs du type virgule "," et deux-points ":" ----#
        # --- On garde les tirets "-" (cf mots composés avec des "-") --------------------------------------------------------#
        # --- * On remplace dans les titres sans stopwords et sans année, les séparateurs du type ' - ' par un espace du type ' ' -----#c
        # --- * on extrait la liste des mots qui composent les titres sans stopwords et sans année ------------------------------------#
        # --- * on nettoie la liste des mots qui composent les titres sans stopwords et sans année ---------------------------#
        # --- en enlevant les blancs qui peuvent etre autour des mots, en mettant en minuscules ----------------------------#
        # --- et en enlevant au début des mots, le pattern du type "' " ------------------------------------------------#
    ...
    print("all_results avec final_result_X_phrases_bis", all_results.shape)
    ...
    print("all_results apres try / except", all_results.shape)
    ...
    if method == "new_one":
        ...
        # print (list(orgc_descriptions['xurl']))

        # print ("orgc_descriptions_new_products", orgc_descriptions_new_products.shape)
        # Pas besoin de faire ça
        # idx_of_url = list(orgc_descriptions_new_products[orgc_descriptions_new_products['xurl'].isin(list(orgc_descriptions_new_products['xurl']))].index.values.astype(int)[0])
        # print ("idx_of_url", idx_of_url)

        # slug_of_url = orgc_descriptions[orgc_descriptions['xurl'] == url_product].loc[idx_of_url, 'xslug']
        # Pas besoin de faire ça
        # slug_of_url = orgc_descriptions_new_products[orgc_descriptions_new_products['xurl'].isin(list(orgc_descriptions_new_products['xurl']))].loc[idx_of_url, 'xslug']

        # print("slug_of_url", list(orgc_descriptions_new_products['xslug']))
    ...
    if method == "all_except_one":
        if threshold != None:
            #output_file_name = 'all_results_BIO_SCRIPT_PYCHARM_with_exhaust_and_titles_equal_' + datetime.datetime.today().strftime("%d%m%y") + '_ALL_PRODUCTS_without_new_product_' + str(threshold) + '.xlsx'
            output_file_name = 'all_results_BIO_SCRIPT_PYCHARM_with_exhaust_and_titles_equal_' + datetime.datetime.today().strftime("%Y%m%d") + '_' + method + '_' +  str(threshold) + '.xlsx'
            ...
            # output_file_name = 'all_results_BIO_SCRIPT_PYCHARM_with_exhaust_and_titles_equal_' + datetime.datetime.today().strftime("%d%m%y") + '_ALL_PRODUCTS_without_new_product.xlsx'
            output_file_name = 'all_results_BIO_SCRIPT_PYCHARM_with_exhaust_and_titles_equal_' + datetime.datetime.today().strftime("%Y%m%d") + '_' +  method + '.xlsx'
            # Sauvegarde en .pkl pour 3ème cas
    ...
    # elif path_input != None and method == "new_one":
        ...
        if threshold != None:
            # output_file_name = 'all_results_BIO_SCRIPT_PYCHARM_with_exhaust_' + datetime.datetime.today().strftime("%d%m%y") + '_NEW_PRODUCT_' + str(threshold) + '.xlsx'
            output_file_name = 'all_results_BIO_SCRIPT_PYCHARM_with_exhaust_' + datetime.datetime.today().strftime("%Y%m%d") + '_' +  method + '_' +  str(threshold) + '.xlsx'
            # Sauvegarde en .pkl pour 3ème cas
            ...
            output_file_name = 'all_results_BIO_SCRIPT_PYCHARM_with_exhaust_' + datetime.datetime.today().strftime("%Y%m%d") + '_' +  method + '.xlsx'
            # Sauvegarde en .pkl pour 3ème cas
    ...
    # elif path_input != None and method == "all":
        ...
        if threshold != None:
            ...
            output_file_name = 'all_results_BIO_SCRIPT_PYCHARM_with_exhaust_and_titles_equal_' + datetime.datetime.today().strftime("%Y%m") + '_' + method + '.xlsx'
...
if __name__== "__main__":
    ...
    # 07/05/2019: plus besoin de "path_input_files"
